...
```
spectroscopy/
├── LICENSE                          # MIT License — Teravation LLC
...
├── src/
│   ├── physics/
│   │   ├── types.ts                 # SpectralLine, Element, Puzzle, PuzzleSettings + hasRenderableLines()
...
│   ├── data/
│   │   ├── fetchElements.ts         # fetch + parse elements.json; returns { elements, datasetDate }
│   │   └── useElements.ts           # React Query hook wrapping fetchElements
│   ├── components/
│   │   ├── AppHeader.tsx            # Branding row (44px) + optional top AdSlot; contains ☰ trigger
│   │   ├── AppMenu.tsx              # Hamburger slide-in drawer; all game controls + version info
│   │   ├── NewTargetDialog.tsx      # Settings dialog opened by "New Target" in AppMenu
│   │   ├── PageShell.tsx            # Shared layout for non-game pages (/about, /licenses)
│   │   ├── SpectrumCanvas.tsx       # Canvas pixel-scan renderer (port of SpectrumPanel.java)
│   │   ├── PeriodicTable.tsx        # CSS Grid table; zoom mode via prop; emits hover via callback
│   │   ├── DopplerSlider.tsx        # range [-100..100] → [-1.0..1.0] with exact labels
│   │   ├── AdSlot.tsx               # Conditional AdSense slot placeholder (Phase 6)
│   │   └── Toast.tsx                # Transient feedback (correct/incorrect/hint) in target canvas
│   ├── pages/
│   │   ├── about.md                 # About page content (edit this to update /about)
│   │   ├── AboutPage.tsx            # Renders about.md via MDX; custom component renderers
│   │   └── LicensesPage.tsx         # Fetches /THIRD_PARTY_LICENSES.txt (generated at build)
│   ├── types/
│   │   └── raw.d.ts                 # TypeScript declaration for *.md MDX imports
│   ├── store/
│   │   └── gameStore.ts             # Zustand store
│   ├── branding/
│   │   └── useBranding.ts           # Brand config loader (subdomain or ?brand= param) — Phase 6
│   └── index.css                    # CSS custom properties + global styles + page styles
├── public/
│   ├── manifest.json
│   └── THIRD_PARTY_LICENSES.txt     # gitignored; generated by licensesPlugin() on every build/dev start
├── index.html
├── vite.config.ts                   # MDX plugin + licensesPlugin() + React
└── package.json
```

**Routes (React Router v7):**
- `/` — the game
- `/about` — science primer, app history, credits
- `/licenses` — auto-generated third-party license text

**CloudFront:** custom error responses 403+404 → `/index.html` (200) so all routes are directly navigable and SEO-indexable.
